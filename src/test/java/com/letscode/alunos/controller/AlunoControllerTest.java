...
/**
 * WebMvcTest -> é a anotação usada para iniciar o contexto de aplicação que contém apenas os beans
 * necessários para testar um controller
 */

/**
 * Estamos passando o AlunoController na anotação para restringir o contexto da aplicação se não
 * especificarmos o Spring Boot irá incluir todos o controllers no contexto da aplicação. Assim,
 * precisamos incluir ou simular todos os beans dos quais qualquer controller depende
 */
...
class AlunoControllerTest {
  ...
  /**
   * Agora podemos @Autowired todos os beans que precisamos do contexto do aplicativo. O Spring
   * fornece automaticamente beans como MockMvc, ObjectMapper
   */

  /**
   * Usamos a anotação @MockBean para simular a logica de negocios (service), pois não queremos
   * testar a integração do controller com o serviço, mas sim entre o controller e o HTTP. @MockBean
   * substitui automaticamente o bean do mesmo tipo no contexto do aplicativo por um mock do
   * Mockito.
   */

  /**
   * Diferença Os campos anotados com @Mock serão inicializados automaticamente com uma instância
   * simulada de seu tipo, assim como chamaríamos Mockito.mock() manualmente. A anotação @MockBean
   * fará com que o Spring procure um bean do tipo existente AlunoService no contexto do aplicação
  ...
  void setup() {
    ...
    aluno = new Aluno(1L, "Jonathan2", 22L, "4354354333", "Rua teste 2242");
  }
  ...
  void deveVerificarOValorDoNomeDoAlunoQueFoiEnviado() throws Exception {
    ...
    // ArgumentCaptor para capturar o Aluno objeto que foi enviado para o metodo salvar e afirmar
    // que ele contém os valores esperados.
  ...
  void deveBuscarTodosOsAlunos() throws Exception {
    ...
    MvcResult result =
        mockMvc
            .perform(
                get("/alunos") // especifica o metodo http que será chamado passando a url
  ...
  void deveRetornarUmaListaVazia() throws Exception {
    ...
    MvcResult result =
        mockMvc
            .perform(get("/alunos").contentType(MediaType.APPLICATION_JSON))
  ...
  void deveRetornaOAlunoQuandoBuscarPorId() throws Exception {
    ...
    MvcResult result =
        mockMvc
            .perform(get("/alunos/{id}", aluno.getId()).contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk())
            .andReturn();
  ...
  public void deveRetornarOAlunoQuandoBuscarPorIdade() throws Exception {

    List<Aluno> alunos = Arrays.asList(aluno);
    when(alunoService.buscaPorIdade(anyLong())).thenReturn(alunos);

    MvcResult resultado =
        mockMvc
            .perform(
                get("/alunos/idades/{idade}", aluno.getIdade())
  ...
  public void deveBuscarPorNome() throws Exception {
    ...
    when(alunoService.buscaPorNome(aluno.getNome())).thenReturn(alunos);

    MvcResult resultado =
        mockMvc
            .perform(
                get("/alunos/nomes/{nome}", aluno.getNome())
                    .contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk())
            .andReturn();
